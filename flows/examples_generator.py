# EDITED FROM: https://github.com/gempy-project/gempy/blob/main/gempy/API/examples_generator.py
# For OneCode
...
def generate_example_model(example_model: ExampleModel, compute_model: bool = True, resolution: List[int] = [50, 50, 50], refinement: int = 2) -> gp.data.GeoModel:
    match example_model:
        case ExampleModel.HORIZONTAL_STRAT:
            return _generate_horizontal_stratigraphic_model(compute_model, resolution, refinement)
        case ExampleModel.ANTICLINE:
            return _generate_anticline_model(compute_model, resolution, refinement)
        case ExampleModel.ONE_FAULT:
            return _generate_one_fault_model(compute_model, resolution, refinement)
        case ExampleModel.TWO_AND_A_HALF_D:
            return _generate_2_5d_model(compute_model, resolution, refinement)
        case ExampleModel.COMBINATION:
            return _generate_combination_model(compute_model, resolution, refinement)
        case ExampleModel.ONE_FAULT_GRAVITY:
            return _generate_one_fault_model_gravity(compute_model, resolution, refinement)
        case ExampleModel.GRABEN:
            return _generate_graben_model(compute_model, resolution, refinement)
        case _:
            raise NotImplementedError(f"Example model {example_model} not implemented.")


def _generate_2_5d_model(compute_model: bool, resolution: List[int] = [50, 50, 50], refinement: int = 1) -> gp.data.GeoModel:
...
def _generate_horizontal_stratigraphic_model(compute_model: bool, resolution: List[int] = [50, 5, 50], refinement: int = 3) -> gp.data.GeoModel:
    """
...
def _generate_anticline_model(compute_model: bool, resolution: List[int] = [50, 50, 50], refinement: int = 5) -> gp.data.GeoModel:
    """
...
def _generate_one_fault_model(compute_model: bool, resolution: List[int] = [20, 20, 20], refinement: int = 6) -> gp.data.GeoModel:
    """
...
def _generate_combination_model(compute_model: bool, resolution: List[int] = [50, 50, 50], refinement: int = 4) -> gp.data.GeoModel:
    """
...
def _generate_one_fault_model_gravity(compute_model, resolution: List[int] = [150, 10, 150], refinement: int = 2):
...
def _generate_graben_model(compute_model: bool, resolution: List[int] = [50, 50, 50], refinement: int = 6) -> gp.data.GeoModel:
    ...
    geo_data: gp.data.GeoModel = gp.create_geomodel(
        ...
        resolution=resolution,
        refinement=refinement,  # * For this model is better not to use octrees because we want to see what is happening in the scalar fields
        importer_helper=gp.data.ImporterHelper(
